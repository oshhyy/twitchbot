...
exports.handle = async msg => {
    ...
    const { prefix } = await bot.db.channels.findOne({id: msg.channel.id});
    ...
    try {
        ...
        if (bannedChannels.includes(msg.user.id)) return;
        ...
        if (offlineOnlyChannels.includes(msg.channel.id)) {
            let data = await got(`https://api.ivr.fi/v2/twitch/user?login=${msg.channel.login}`).json();
        ...
        if (!msg.botBadges.badges.hasModerator && !msg.botBadges.badges.hasVIP && msg.user.id === '489223884' && delay < 900) { await bot.Utils.sleep(900 - delay); }

        if (bot.Utils.regex.slurs.test(result.text) || bot.Utils.regex.tos.test(result.text) || bot.Utils.regex.racism.test(result.text)) {
            await bot.Client.sendRaw(`@reply-parent-msg-id=${reply} PRIVMSG #${msg.channel.login} :Message contains banned phrase! elisBruh`)
            bot.Webhook.error(`ban phrase detected • ${command.name} by ${msg.user.login} in #${msg.channel.login}`, `${msg.message.text} \n\n${result.text}`);
            return;
        }
